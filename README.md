
🌊 The Ocean Code 🐳

✨ Projektbeskrivelse
The Ocean Code er et læringsspil, der bygger på strukturen fra World of Zuul og fokuserer på FN's verdensmål for livet i havet. Spillet er designet til interaktivt at undervise spillere om havforurening og understrege den kritiske vigtighed af at beskytte vores oceaner.

🎮 Om Spillet
I The Ocean Code får du rollen som havets beskytter, hvor du skal navigere gennem forskellige marine områder for at udføre vitale opgaver:
* Samle plastik 🗑️
* Rense koralrev 🐠
* Klippe fiskenet ✂️

Under din rejse vil du støde på quizzer, der udfordrer og tester din viden om havmiljøet og dets forureningsproblematikker. Spillet leverer en unik kombination af underholdning og uddannelse, der hjælper dig med at forstå, hvordan vi aktivt kan beskytte vores havområder.

🎯 Formål og Læringsmål
Spillet er skabt med det formål at bidrage til en bedre forståelse af havets udfordringer:
* Øge bevidstheden om havforurening 🏭
* Undervise i FN's verdensmål for livet i havet (NR.14) 🌐
* Give en interaktiv og engagerende læringsoplevelse 💡

🚀 Kom godt i gang (Tutorial)
Følg disse trin for at downloade og køre The Ocean Code lokalt:
1. Forudsætninger: Sørg for at have følgende installeret:
    * Avalonia 🖼️
    * .NET 9.0 SDK 💻
2. Kørsel: Naviger til projektmappen "Presentation" og indtast følgende kommando:

dotnet run


